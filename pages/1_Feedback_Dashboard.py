...
<div style="background:linear-gradient(135deg,#1a3c5e 0%,#0d7b6e 100%);
  ...
  <p style="margin:0;opacity:0.85;font-size:0.9rem;">Loan Readiness Platform — User Feedback Analytics</p>
</div>
...
if df.empty:
    st.info("No feedback collected yet. Feedback will appear here once users submit it.")
    st.stop()
...
if not stats.empty:
    ...
    chart_df = stats.set_index("step_name")[["avg_overall", "avg_ease", "avg_usefulness"]].rename(columns={
        "avg_overall":    "Overall",
    ...
    st.markdown("#### Response counts per step")
    display_stats = stats[["step_name", "responses", "avg_overall", "avg_ease", "avg_usefulness"]].rename(columns={
        "step_name":       "Step",
        "responses":       "Responses",
...
awareness_counts = df["loan_awareness"].value_counts().reset_index()
awareness_counts.columns = ["Answer", "Count"]
c1, c2 = st.columns([1, 2])
with c1:
    st.dataframe(awareness_counts, use_container_width=True, hide_index=True)
with c2:
    st.bar_chart(awareness_counts.set_index("Answer"), height=220)
...
rating_dist["Stars"] = rating_dist["Stars"].apply(lambda x: f"{'★'*x}{'☆'*(5-x)} ({x})")
st.bar_chart(rating_dist.set_index("Stars"), height=250)
